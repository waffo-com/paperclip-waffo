...
export function agentService(db: Db) {
  ...
  return {
    ...
    create: async (companyId: string, data: Omit<typeof agents.$inferInsert, "companyId">, options?: CreateAgentOptions) => {
      ...
      // Feeds rawAdapterConfig rather than assigning adapterConfig directly:
      // normalizePaperclipRunnerAdapterConfig still has to run below, and the
      // Claude OAuth binding invariant is asserted against its output.
      const rawAdapterConfig = Object.keys(adapterConfigWithDefaults).length > 0
