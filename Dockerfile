...
#
# Ordered BEFORE the build-commit ARG on purpose: that ARG changes on every
# commit, and a changed ARG busts the cache for every layer after it, so this
# bundle would be rebuilt on each build for no reason.
